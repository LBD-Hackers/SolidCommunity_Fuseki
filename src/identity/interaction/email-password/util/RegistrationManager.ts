...
export class RegistrationManager {
  ...
  public async register(input: RegistrationParams, allowRoot = false): Promise<RegistrationResponse> {
    ...
    await this.createRepository(input.webId!)
    // Create the pod
    if (input.createPod) {
  ...
  private async createRepository(webId: string) {
    ...
    fetch(`${process.env.SPARQL_STORE_ENDPOINT}/$/datasets?dbName=${name}&dbType=tdb2`, requestOptions)
      ...
      .then(result => this.logger.info("Created pod mirror on remote SPARQL endpoint"))
